...
# Run name and paths
RUN_NAME="elliptical-shape"            # Run name
...
DOMAINSIZE=12000.0                        # Height and width of domain (m)
A_SRS="EPSG: 32610"                       # Spatial reference system

# Simulation timing
SIMULATION_TSTOP=25170.0                  # Simulation stop time (s)
DTDUMP=3600                               # Output dump interval (s)

# Fuel/Weather raster data
NUM_FLOAT_RASTERS=7
FLOAT_RASTER[1]=ws   ; FLOAT_VAL[1]=10.0  # Wind speed, (mph)
FLOAT_RASTER[2]=wd   ; FLOAT_VAL[2]=180.0 # Wind direction, (deg)
FLOAT_RASTER[3]=m1   ; FLOAT_VAL[3]=3.0   # 1-hr   dead moisture content (%)
FLOAT_RASTER[4]=m10  ; FLOAT_VAL[4]=3.0   # 10-hr  dead moisture content (%)
FLOAT_RASTER[5]=m100 ; FLOAT_VAL[5]=3.0   # 100-hr dead moisture content (%)
...
INT_RASTER[5]=cc      ; INT_VAL[5]=0      # Canopy cover (%)
INT_RASTER[6]=ch      ; INT_VAL[6]=0      # Canopy height (10*m)
INT_RASTER[7]=cbh     ; INT_VAL[7]=0      # Canopy base height (10*m)
