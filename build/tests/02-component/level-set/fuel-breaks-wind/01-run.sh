...
# Run Name: fuel-breaks-wind
# Questions? Contact: adam.laird@berkeley.edu

# Note: This test case matches the verification study provided in a study by
# Bova, Mell, and Hoffman. http://dx.doi.org/10.1016/j.firesaf.2013.08.014
# Some inputs have been changed to match formatting of ELMFIRE.
...
# Run name and paths
RUN_NAME="four-fuels-nwns"                # Run name
...
DOMAINSIZE=1000                           # Height and width of domain (m)
A_SRS="EPSG: 32610"                       # Spatial reference system

# Simulation timing
SIMULATION_TSTOP=7200.0                   # Simulation stop time (s)
DTDUMP=300.0                             # Output dump interval (s)

# Fuel/Weather raster data
NUM_FLOAT_RASTERS=7
FLOAT_RASTER[1]=ws   ; FLOAT_VAL[1]=11.1846815  # Wind speed (mph)
FLOAT_RASTER[2]=wd   ; FLOAT_VAL[2]=270.0  # Wind direction (deg)
FLOAT_RASTER[3]=m1   ; FLOAT_VAL[3]=2.0   # 1-hr   dead moisture content (%)
FLOAT_RASTER[4]=m10  ; FLOAT_VAL[4]=2.0   # 10-hr  dead moisture content (%)
FLOAT_RASTER[5]=m100 ; FLOAT_VAL[5]=2.0   # 100-hr dead moisture content (%)
...
INT_RASTER[4]=cc      ; INT_VAL[4]=0      # Canopy cover (%)
INT_RASTER[5]=ch      ; INT_VAL[5]=0      # Canopy height (10*m)
INT_RASTER[6]=cbh     ; INT_VAL[6]=0      # Canopy base height (10*m)
INT_RASTER[7]=cbd     ; INT_VAL[7]=0      # Canopy bulk density (100*kg/m3)
...
echo "Generating custom fuels raster..."
python3 ./02-build-fbfm.py "./fbfm40_map.csv" "$INPUTS/fbfm40.tif" "$DOMAINSIZE" "$CELLSIZE"
