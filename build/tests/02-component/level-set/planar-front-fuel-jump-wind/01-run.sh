...
# Run Name: planar-front-fuel-jump
...
# Run name and paths
RUN_NAME="planar-front-fuel-jump"      # Run name
...
DOMAINSIZE=1500                           # Height and width of domain (m)
A_SRS="EPSG: 32610"                       # Spatial reference system

# Simulation timing
SIMULATION_TSTOP=32000.0                  # Simulation stop time (s)
DTDUMP=1200.0                             # Output dump interval (s)

# Fuel/Weather raster data
NUM_FLOAT_RASTERS=6
FLOAT_RASTER[1]=ws   ; FLOAT_VAL[1]=1.0   # Wind speed (mph)
FLOAT_RASTER[2]=wd   ; FLOAT_VAL[2]=270.0 # Wind direction (deg)
...
INT_RASTER[4]=cc      ; INT_VAL[4]=70     # Canopy cover (%)
INT_RASTER[5]=ch      ; INT_VAL[5]=400    # Canopy height (10*m)
INT_RASTER[6]=cbh     ; INT_VAL[6]=20     # Canopy base height (10*m)
INT_RASTER[7]=cbd     ; INT_VAL[7]=15     # Canopy bulk density (100*kg/m3)

LH_MOISTURE_CONTENT=30.0                  # Live herb. moisture content (%)
LW_MOISTURE_CONTENT=60.0                  # Live woody moisture content (%)

FBFM40_LHS=102            # Fire behavior fuel model code for left  domain (-)
FBFM40_RHS=145            # Fire behavior fuel model code for right domain (-)
...
# Generate fuel/weather/ignition rasters
...
# Create PHI field with vertical ignition line
PHI_LINE_VAL=-1.0
PHI_OTHER_VAL=1.0
PIX_LINE=$(echo "7 * $DOMAINSIZE / $CELLSIZE / 16" | bc)  # domain location
gdal_calc.py -A $SCRATCH/float.tif \
    --co="COMPRESS=DEFLATE" --co="ZLEVEL=9" \
    --NoDataValue=-9999 \
    --type=Float32 \
    --overwrite \
    --calc="numpy.where(numpy.indices(A.shape)[1] == ${PIX_LINE}, ${PHI_LINE_VAL}, ${PHI_OTHER_VAL})" \
    --outfile="$INPUTS/phi.tif"

# Create special fbfm40 input raster
PIX_HALF=$(echo "$DOMAINSIZE / $CELLSIZE / 2" | bc)  # no of columns in left half
gdal_calc.py -A $SCRATCH/int.tif \
   --co="COMPRESS=DEFLATE" --co="ZLEVEL=9" \
   --NoDataValue=-9999 \
   --outfile="$INPUTS/fbfm40.tif" \
   --type=Int16 --overwrite \
   --calc="numpy.where(numpy.indices(A.shape)[1] < $PIX_HALF, $FBFM40_LHS, $FBFM40_RHS)"
