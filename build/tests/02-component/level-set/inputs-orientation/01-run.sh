...
# Run name and paths
RUN_NAME="inputs-orientation"                # Run name
WORKDIR="."                                  # Working directory path
INPUTS_ORIGINAL="$WORKDIR/inputs_original"   # Original Inputs directory path
INPUTS_ROTATED="$WORKDIR/inputs_rotated"     # Rotated inputs directory path
INPUTS_TEMPORARY="$WORKDIR/inputs"           # Temporary inputs directory path
OUTPUTS_ORIGINAL="$WORKDIR/outputs_original" # Original outputs directory path
OUTPUTS_ROTATED="$WORKDIR/outputs_rotated"   # Rotated outputs directory path
...
rm -f -r $INPUTS_TEMPORARY $INPUTS_ORIGINAL \
         $INPUTS_ROTATED $OUTPUTS_ORIGINAL $OUTPUTS_ROTATED $SCRATCH
mkdir -p $INPUTS_ORIGINAL $INPUTS_ROTATED $OUTPUTS_ORIGINAL $OUTPUTS_ROTATED $SCRATCH

for CASE in 1 2; do  # Loop through original & rotated cases

   if [ "$CASE" -eq 1 ]; then
      echo "Running Original Inputs Case..."
      OUTPUTS=$OUTPUTS_ORIGINAL
      INPUTS=$INPUTS_ORIGINAL
      PNG_NAME=fireline_original.png
   else
      echo "Running Rotated Inputs Case..."
      OUTPUTS=$OUTPUTS_ROTATED
      INPUTS=$INPUTS_ROTATED
      PNG_NAME=fireline_rotated.png
   fi

   echo "Creating temporary directories..."
   mkdir -p $INPUTS_TEMPORARY

   echo "Copying elmfire.data.in into $INPUTS..."
   cp $WORKDIR/elmfire.data.in $INPUTS/elmfire.data
   cp $PNG_NAME $INPUTS/$PNG_NAME
   ...
   # Generate PHI field from custom build script
   echo "Generating custom PHI ignition raster..."
   PIXELS=$(echo "$DOMAINSIZE / $CELLSIZE" | bc)  # 400 for 2000m / 5m
   python3 ./02-build-phi.py "$INPUTS/$PNG_NAME" "$INPUTS/phi.tif" "$PIXELS" "$CELLSIZE" "$A_SRS"

   # Move to temporary directory
   rm -rf $INPUTS_TEMPORARY
   mkdir -p $INPUTS_TEMPORARY
   cp -r "$INPUTS/." "$INPUTS_TEMPORARY/"
   ...
   replace_line FUELS_AND_TOPOGRAPHY_DIRECTORY "$INPUTS" yes
   replace_line WEATHER_DIRECTORY "$INPUTS" yes
   replace_line OUTPUTS_DIRECTORY "$OUTPUTS" yes

   # Move back from temporary directory
   rsync -a --delete "$INPUTS_TEMPORARY/"  "$INPUTS/"
   rm -rf $INPUTS_TEMPORARY
