...
# 02-build-exact.sh
# Generates exact solution isochrones for 03-dual-ignition-nwns
# Questions? Contact: adam.laird@berkeley.edu

set -euo pipefail

# Case parameters
IGN1_X=-50.0            # Ignition 1 x (m)
IGN1_Y=0.0              # Ignition 1 y (m)
IGN2_X=50.0             # Ignition 2 x (m)
IGN2_Y=0.0              # Ignition 2 y (m)

SIMULATION_TSTOP=14400   # Simulation stop time (s)
...
./04-exact-shapefiles.py "$R0_ms" "$DTDUMP" "$SIMULATION_TSTOP" "$IGN1_X" "$IGN1_Y" "$IGN2_X" "$IGN2_Y" "$OUTDIR"
