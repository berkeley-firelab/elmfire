...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, K, LU, IT1, IT2, &
           ...
           N_TO_TAG, N_SPOT_FIRES, IT2_LSP, ICOL, IROW
...
REAL :: T_LAST_EXTENDED_ATTACK, T_LAST_INTERPOLATE_M1, T_LAST_INTERPOLATE_M10, T_LAST_INTERPOLATE_M100, &
        ...
        T_LAST_SMOKE_OUTPUT, QDOTNOW, XCEN, YCEN, THI, TLO, MDOTNOW, RADIUS, RUNTIME, FLIN
   ...
   IF (USE_BLDG_SPREAD_MODEL .AND. BLDG_SPREAD_MODEL_TYPE .EQ. 2) THEN
      ...
      DO IX=1,NX
         IF (FBFM%I2(IX,IY,1) .EQ. 91) THEN
            FUEL_LOAD_REMAIN(IX,IY) = BUILDING_FUEL_MODEL_TABLE(BLDG_FUEL_MODEL%I2(IX,IY,1))%FUEL_LOAD
         ENDIF
      ENDDO
      ENDDO
...
continue
   ...
   DO ISTEP=1,2
      ...
      CALL UX_AND_UY_ELLIPTICAL(LIST_BURNED, 1.0, ISTEP, T)
   ENDDO
   ...
   IF (NUM_IGNITIONS .GT. 0) THEN
      DO I = 1, NUM_IGNITIONS
         ...
         IF (T .GE. T_IGN(I)) THEN
            ...
            CALL TAG_BAND(NX,NY,IX_IGN,IY_IGN,T)
            CALL APPEND(LIST_BURNED,IX_IGN,IY_IGN,T)
            LIST_BURNED%TAIL%TIME_OF_ARRIVAL = T-DT
            PHIP    (IX_IGN,IY_IGN) = -1.0
            TIME_OF_ARRIVAL(IX_IGN,IY_IGN) = T-DT
            IF (USE_BLDG_SPREAD_MODEL .AND. BLDG_SPREAD_MODEL_TYPE .EQ. 2) THEN
               ! Tag WUI cells
               CALL TAG_WUI(NX, NY, IX_IGN, IY_IGN, T) 
            ENDIF
...
! Interpolate / map transient weather rasters (modified to update wind field for LIST_WUI_BURNING)
...
! Find newly-burned cells and call spotting:
   ...
   DO I = 1, LIST_TAGGED%NUM_NODES
      ...
      IF (PHIP(IX,IY) .LE. 0. .AND. SURFACE_FIRE(IX,IY) .EQ. 0) THEN
         ...
         C%TIME_OF_ARRIVAL      = T
         SURFACE_FIRE   (IX,IY) = 1
         IF (TIME_OF_ARRIVAL(IX,IY) .LT. 0) TIME_OF_ARRIVAL(IX,IY) = T
...
! #endif
         ...
         IF (ENABLE_SPOTTING .AND. USE_SUPERSEDED_SPOTTING .AND. (.NOT. USE_UMD_SPOTTING_MODEL)) THEN
            ...
            IF(C%IFBFM .EQ. 91) THEN
               FLIN = HRR_TRANSIENT_MAP(C%IX,C%IY)*ANALYSIS_CELLSIZE
            ELSE
               FLIN = C%FLIN_SURFACE
            ENDIF
   ...
   IF(USE_BLDG_SPREAD_MODEL .AND. BLDG_SPREAD_MODEL_TYPE .EQ. 1) THEN
      C => LIST_BURNED%HEAD
   ...
   IF (ENABLE_SPOTTING .AND. USE_UMD_SPOTTING_MODEL .AND. (.NOT. USE_SUPERSEDED_SPOTTING)) THEN
      ...
      DO I = 1, LIST_BURNED%NUM_NODES
         CALL_SPOTTING = .FALSE.
         CALL CALC_SPOTTING_DURATION(C, T)
...
CALL ACCUMULATE_CPU_USAGE(63, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(64, IT1, IT2)
...
#ifdef _UMDSPOTTING
! *****************************************************************************
SUBROUTINE EULERIAN_SPOTTING_MAIN(NX_ELM, NY_ELM, CELLSIZE_ELM, T_ELMFIRE, DT_ELMFIRE, WS20)
! *****************************************************************************
! Main call to ember trajectory integration and ignition determination
USE ELMFIRE_VARS
...
DO
   ...
   IF (.NOT. ASSOCIATED(C)) EXIT
   CALL EMBER_TRAJECTORY_EULERIAN(NX_ELM, NY_ELM, CELLSIZE_ELM, C, T_ELMFIRE, DT_ELMFIRE)
...
! Ignite firebrand-landed pixels
...
DO 
   ...
   IX = C%IX
   IY = C%IY
   CALL EMBER_CONSUMPTION(IX, IY, T_ELMFIRE, DT_ELMFIRE)
   ...
   IF(PHIP(IX,IY) .GE. 0 .AND. SURFACE_FIRE(IX,IY) .LE. 0) THEN
      IF (USE_EMBER_IGNITION_MODEL) THEN
         ...
         IF (.NOT. EMBER_IGNITION(IX,IY,T_ELMFIRE, DT_ELMFIRE, WS20, LOCAL_IGNITION_TIME, CELL_IGNITION_DELAY)) THEN
            C => C%NEXT
            CYCLE
         ENDIF
