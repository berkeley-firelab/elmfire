...
! === RNG INIT (NVFORTRAN-SAFE) ===
BLOCK
  USE ISO_FORTRAN_ENV, ONLY: INT64
  INTEGER :: M_LOC, I_LOC, RATE_LOC
  INTEGER, ALLOCATABLE :: K_LOC(:), KGET_LOC(:)
  INTEGER(INT64) :: COUNT64, STATE64, MIXED64

  CALL RANDOM_SEED(SIZE=M_LOC)
  ALLOCATE(K_LOC(M_LOC))

  IF (RANDOMIZE_RANDOM_SEED) THEN
     CALL SYSTEM_CLOCK(COUNT=COUNT64, COUNT_RATE=RATE_LOC)
     IF (RATE_LOC <= 0) RATE_LOC = 1

     ! BUILD A 64-BIT ENTROPY STATE USING CLOCK, RATE, AND MPI RANK
     STATE64 = COUNT64                                         &
        + INT(RATE_LOC, INT64) * 6364136223846793005_INT64     &
        + INT(IRANK_WORLD+1, INT64) * 1442695040888963407_INT64

     DO I_LOC = 1, M_LOC
        ! SPLITMIX64-STYLE MIXING
        MIXED64 = STATE64 + INT(I_LOC, INT64) * 1099511628211_INT64
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64, -30)); MIXED64 = MIXED64 * INT(Z'BF58476D1CE4E5B9', INT64)
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64, -27)); MIXED64 = MIXED64 * INT(Z'94D049BB133111EB', INT64)
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64, -31))

        ! MAP TO NONNEGATIVE DEFAULT INTEGER RANGE
        K_LOC(I_LOC) = INT(IAND(MIXED64, INT(Z'7FFFFFFF', INT64)), KIND(0))
        IF (K_LOC(I_LOC) == 0) K_LOC(I_LOC) = I_LOC*37
     END DO

  ELSE
     ! DETERMINISTIC SCALAR->VECTOR EXPANSION (REPRODUCIBLE)
     DO I_LOC = 1, M_LOC
        K_LOC(I_LOC) = SEED + 104729*(I_LOC-1) + 1009*(IRANK_WORLD+1)
     END DO
  END IF

  CALL RANDOM_SEED(PUT=K_LOC)

  ! OPTIONAL: LOG ACTUAL SEED VECTOR (PER RANK) FOR REPLAY
  ALLOCATE(KGET_LOC(M_LOC))
  CALL RANDOM_SEED(GET=KGET_LOC)
  WRITE(*,'(A,I0,A)') 'RANK ', IRANK_WORLD, ' RANDOM_SEED ='
  WRITE(*,'(10(I12))') (KGET_LOC(I_LOC), I_LOC=1, M_LOC)
  DEALLOCATE(KGET_LOC)
  DEALLOCATE(K_LOC)
END BLOCK
! === END RNG INIT ===
...
IF (MODE .NE. 2) THEN
   ...
   DO WHILE (ICASE .LT. NUM_CASES_TOTAL)
      ...
      IF (IWX_BAND .NE. IWX_BAND_LAST) THEN
         WSP  (1:,1:,1:) => WS%R4  (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         WDP  (1:,1:,1:) => WD%R4  (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         M1P  (1:,1:,1:) => M1%R4  (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         M10P (1:,1:,1:) => M10%R4 (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         M100P(1:,1:,1:) => M100%R4(1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         MLHP (1:,1:,1:) => MLH%R4 (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         MLWP (1:,1:,1:) => MLW%R4 (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         MFOLP(1:,1:,1:) => MFOL%R4(1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         IF (USE_ERC) THEN
            ERCP   (1:,1:,1:) => ERC%R4   (1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
            IGNFACP(1:,1:,1:) => IGNFAC%R4(1:,1:,IWX_BAND-IWX_BAND_OFFSET:)
         ENDIF
      ENDIF
      
      CALL ACCUMULATE_CPU_USAGE(13, IT1, IT2)
      CALL seed_rng_per_case(ICASE, IWX_BAND, IRANK_WORLD, RANDOMIZE_RANDOM_SEED, SEED)   
...
SUBROUTINE seed_rng_per_case(icase, iwx_band, irank_world, randomize, base_seed)
  USE ISO_FORTRAN_ENV, ONLY: INT64
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: icase, iwx_band, irank_world, base_seed
  LOGICAL, INTENT(IN) :: randomize
  INTEGER :: M_LOC, I_LOC, RATE_LOC
  INTEGER, ALLOCATABLE :: K_LOC(:)
  INTEGER(INT64) :: COUNT64, STATE64, MIXED64

  CALL RANDOM_SEED(SIZE=M_LOC); ALLOCATE(K_LOC(M_LOC))
  IF (randomize) THEN
     CALL SYSTEM_CLOCK(COUNT=COUNT64, COUNT_RATE=RATE_LOC); IF (RATE_LOC<=0) RATE_LOC=1
     STATE64 = COUNT64                                              &
        + INT(RATE_LOC,INT64)          * 6364136223846793005_INT64  &
        + INT(irank_world+1,INT64)     * 1442695040888963407_INT64  &
        + INT(icase,INT64)             * 1099511628211_INT64        &
        + INT(iwx_band+7,INT64)        * 4294967291_INT64
     DO I_LOC=1,M_LOC
        MIXED64 = STATE64 + INT(I_LOC,INT64) * 1099511628211_INT64
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64,-30)); MIXED64 = MIXED64 * INT(Z'BF58476D1CE4E5B9',INT64)
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64,-27)); MIXED64 = MIXED64 * INT(Z'94D049BB133111EB',INT64)
        MIXED64 = IEOR(MIXED64, ISHFT(MIXED64,-31))
        K_LOC(I_LOC) = INT(IAND(MIXED64, INT(Z'7FFFFFFF',INT64)), KIND(0)); IF (K_LOC(I_LOC)==0) K_LOC(I_LOC)=37*I_LOC
     END DO
  ELSE
     DO I_LOC=1,M_LOC
        K_LOC(I_LOC) = base_seed + 104729*(I_LOC-1) + 1009*(irank_world+1) + 8191*icase + 1223*(iwx_band+7)
     END DO
  END IF
  CALL RANDOM_SEED(PUT=K_LOC); DEALLOCATE(K_LOC)
END SUBROUTINE
! *****************************************************************************
END PROGRAM ELMFIRE
! *****************************************************************************
