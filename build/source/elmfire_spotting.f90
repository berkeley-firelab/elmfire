...
IF (USE_EULERIAN_SPOTTING) THEN
   ...
   LIST_EMBER_TRACKER%TAIL%SIGMA_DIST     = SIGMA_DIST
   LIST_EMBER_TRACKER%TAIL%MU_DIST        = MU_DIST 
   LIST_EMBER_TRACKER%TAIL%SIGMA_SPANWISE = SIGMA_SPANWISE
   LIST_EMBER_TRACKER%TAIL%MU_SPANWISE    = MU_SPANWISE
   LIST_EMBER_TRACKER%TAIL%TARGET_ARRIVED = .TRUE.
ELSE
...
REAL :: R0, WD1TO, WD2TO, WDTO, WS20, T, DT, DIST_LAST, F_WIND, X_MAX, Y_MAX, P_LAND, P_LAND_SPANWISE, LNORM_QUANTILE, &
...
DO WHILE (T .LT. DT_ELMFIRE)
   ...
   IF (K_MAX_SPANWISE .GT. 1) THEN
      ...
      EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) = EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE
      ...
      DO I=2, (K_MAX_SPANWISE+1)
         ...
         EMBER_FLUX_TRANSIENT%R4(IX_SPANWISE,IY_SPANWISE, 1) = EMBER_FLUX_TRANSIENT%R4(IX_SPANWISE,IY_SPANWISE, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE
      ...
      ENDDO
   ELSE
      EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) = EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) + NUM_EMBERS*P_LAND
