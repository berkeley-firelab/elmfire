...
SUBROUTINE SPOTTING(IX0,IY0,WS20_NOW,FLIN, N_SPOT_FIRES, IX_SPOT_FIRE, IY_SPOT_FIRE, &
                    ICASE, DT, TIME_NOW, TAU, IGNMULT,  IFBFM, LIST_EMBER_TRACKER)
...
IF (USE_EULERIAN_SPOTTING) THEN
   ...
   LIST_EMBER_TRACKER%TAIL%SIGMA_SPANWISE = MAX(SIGMA_SPANWISE, 1E-5)
   LIST_EMBER_TRACKER%TAIL%MU_SPANWISE    = MAX(MU_SPANWISE,    1E-5)
...
REAL, INTENT(IN) :: CELLSIZE_ELM, TIME_NOW, DT_ELMFIRE
...
REAL :: WD1TO, WD2TO, WDTO, WS20, T, DT, DIST_LAST, F_WIND, X_MAX, Y_MAX, P_LAND, P_LAND_SPANWISE, LNORM_QUANTILE, &
        NORM_QUANTILE, QUANTILE, NORM_FACTOR_SPANWISE, NORM_FACTOR, X_SPANWISE, Y_SPANWISE, INV_UWIND_TIMES_SPANWISE_DEVIATION, &
        UWIND_ABS, NUM_EMBERS, SIGMA_DIST, MU_DIST, SIGMA_SPANWISE, MU_SPANWISE
REAL, DIMENSION(3) :: X, X0, UWIND, OFFSET
INTEGER :: IX, IY, IXLAST, IYLAST,ICOL, IROW, ICOUNT, K_MAX, K_MAX_SPANWISE, ITLO_METEOROLOGY, ITHI_METEOROLOGY, I, &
...
MU_DIST    = C%MU_DIST
SIGMA_SPANWISE = C%SIGMA_SPANWISE
MU_SPANWISE    = C%MU_SPANWISE
...
DO WHILE (T .LT. DT_ELMFIRE)
   ...
   IF (K_MAX_SPANWISE .GT. 1) THEN
      ...
      IF (DUMP_EMBER_FLUX_TRANSIENT) EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) = EMBER_FLUX_TRANSIENT%R4(IX,IY, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE
      EMBER_FLUX%R4(IX,IY, 1) = EMBER_FLUX%R4(IX,IY, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE

      IF(EMBER_TIGN(IX,IY) .LT. 0.0) THEN
         EMBER_TIGN(IX,IY) = T+TIME_NOW
         CALL APPEND(LIST_EMBER_DEPOSITED, IX, IY, EMBER_TIGN(IX,IY))
      ELSE
         IF(EMBER_TIGN(IX,IY) .GT. T+TIME_NOW) EMBER_TIGN(IX,IY) = T+TIME_NOW
      ENDIF
      DO I=2, (K_MAX_SPANWISE+1)
         ...
         IF (DUMP_EMBER_FLUX_TRANSIENT) EMBER_FLUX_TRANSIENT%R4(IX_SPANWISE,IY_SPANWISE, 1) = EMBER_FLUX_TRANSIENT%R4(IX_SPANWISE,IY_SPANWISE, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE
         EMBER_FLUX%R4(IX_SPANWISE,IY_SPANWISE, 1) = EMBER_FLUX%R4(IX_SPANWISE,IY_SPANWISE, 1) + NUM_EMBERS*P_LAND*P_LAND_SPANWISE

         IF(EMBER_TIGN(IX_SPANWISE,IY_SPANWISE) .LT. 0.0) THEN
            EMBER_TIGN(IX_SPANWISE,IY_SPANWISE) = T+TIME_NOW
            CALL APPEND(LIST_EMBER_DEPOSITED, IX_SPANWISE,IY_SPANWISE, EMBER_TIGN(IX_SPANWISE,IY_SPANWISE))
         ELSE
            IF(EMBER_TIGN(IX_SPANWISE,IY_SPANWISE) .GT. T+TIME_NOW) EMBER_TIGN(IX_SPANWISE,IY_SPANWISE) = T+TIME_NOW
         ENDIF
         ! Side-2
         INV_UWIND_TIMES_SPANWISE_DEVIATION = -CELLSIZE_ELM*(I-1)/MAX(1E-6,UWIND_ABS)
         X_SPANWISE=X(1)+(COS(90*PI/180.)*UWIND(1)-SIN(90*PI/180.)*UWIND(2))*INV_UWIND_TIMES_SPANWISE_DEVIATION
...
! *****************************************************************************
LOGICAL FUNCTION EMBER_IGNITION(IX,IY,T_ELMFIRE, DT_ELMFIRE, UWIND, TAU_IGN_INPUT, T_DEVELOP_INPUT)
! *****************************************************************************
! Firebrand ignition model, based on the ember accumulation history
USE ELMFIRE_VARS

REAL, INTENT(IN) :: T_ELMFIRE, DT_ELMFIRE, UWIND, TAU_IGN_INPUT, T_DEVELOP_INPUT
...
! *****************************************************************************
SUBROUTINE CALC_SPOTTING_DURATION(C, T_ELMFIRE)
! *****************************************************************************
